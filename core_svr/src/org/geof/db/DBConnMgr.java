...
//	private int _connectCount = 0;
	...
	private synchronized void checkPools() {
		...
		try {
//			GLogger.verbose("checkPools locking");
			_poolLocked = true;
...
//					GLogger.verbose("Killing long running dbconn: " + conn2.getID());
					killList.add(conn2);
				}
			}
...
//			GLogger.verbose("checkPools killList complete");
...
//			GLogger.verbose("checkPools Remove idle complete");
...
//			GLogger.verbose("checkPools Fill pool complete");
		} catch (Exception e) {
			GLogger.error(e);
		} finally {
			_poolLocked = false;	
//			GLogger.verbose("... checkPools unlocked");
		}
	}
	...
	public static DBConnection getConnection(GSession session) {
		DBConnection conn = DBConnMgr.get().getDBConnection();
	...
	private synchronized DBConnection getDBConnection() {
		try {
			if (_poolLocked) {
				GLogger.debug("DBConnMgr.getConnection() _poolLocked");
				return null;
			}
			...
			if (idleSize > 0) {
				...
				GLogger.error("DBConnMgr.getConnection() maxCreateAttempts reached: " + _failedCreateAttempts);
			}
			else if (_allPool.size() < _maxConnects) {
				conn = createConnection(true);				
			} 
			else {
				GLogger.error("DBConnMgr.getConnection() maximum connections reached: " + _maxConnects);
			}
//			GLogger.verbose("getDBConnection: " + conn.getID());
			return conn;
		} catch (Exception e) {
			GLogger.error("DBConnMgr.getConnection ");
			GLogger.error( e);
			return null;
		}
	}
	...
	private synchronized DBConnection createConnection(boolean inUse) {
		...
		try {
			...
			if (conn.connect(_connStr, _uid, _pwd)) {
				...
				GLogger.error("Database conn.connect failed");
				GLogger.error("_connStr " + _connStr);
				GLogger.error("_uid " + _uid);
				GLogger.error("_pwd " + _pwd);
			}

		} catch (Exception e) {
			GLogger.error("DBConnMgr.createConnection");
			GLogger.error(e);
		}
	...
	public synchronized void close(DBConnection conn) {
		if (conn != null) {
			...
			_allPool.remove(conn);
			//GLogger.verbose("Killing Connection - id=" + conn.getID());
			conn.disconnect();
			conn = null;
		}
	}
	...
	@Override
	public void run() {
		GLogger.writeInit("*** Started - DBConnManager - *** ");
