...
//							GLogger.debug("child added " + em.getName() + " to " + EntityName);
						}
					}					
				}
			}
			...
			GLogger.writeInit("*** EntityMapMgr.initialized ");
			return true;
		} catch (Exception e) {
			GLogger.error(e);
			GLogger.writeInit("*** EntityMapMgr.initialization Failed ");
			return false;
		} finally {
			if (rs != null) {
	...
	public static EntityMap loadMap(String Entityname, DBInteract dbi) {
		...
		try {
			...
			if (rs.next()) {
				...
				GLogger.error("EntityMapManager.loadMap ERROR: " + Entityname + " is not a Entity");
//				GLogger.debug(Thread.currentThread().getStackTrace());
			}
		} catch (Exception e) {
			GLogger.error(e);
		} finally {
	...
	public static void logEntities() {
		...
		GLogger.info("--- EntityMapManager Entity Listing ---");
		while (keys.hasNext()) {
			String key = keys.next();
			GLogger.info(key + " (has map: " + hasMap(key) + ")");
		}
	}
}
