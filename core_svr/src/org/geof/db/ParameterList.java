...
public class ParameterList {
	 ...
	 * @throws Exception 
	 */
	public ParameterList(EntityMap emap, JSONObject fields, ArrayList<String> exclude, ArrayList<String> required) throws Exception {
		this(emap);
			...
			if (required != null) {
				for (String fieldname : required) {
					if (!map.containsKey(fieldname)) {
						throw new Exception("missing fieldname: "  + fieldname + " for " + emap.getName());
					}
				}
			}
		...
		for (String fieldname : fldnames) {
			if (_emap.hasField(fieldname)) {
				...
				if ((!map.containsKey(fieldname)) && (namevalue != null)) {
					// GLogger.error("adding field: " + fieldname);
	...
	public boolean setPreparedStatement(PreparedStatement ps) throws Exception {
		...
		while (rtn && indx < size) {
			rtn = setData(ps, _params.get(indx), indx + 1);
			indx++;
		}
		return rtn;
	}
	...
	protected NameValue addJSON(JSONObject json, String fieldname) throws Exception {
		...
		if ( iDatatype == java.sql.Types.CHAR) {
			...
			if ((value != null) && (! value.equalsIgnoreCase("null"))) {
				java.util.Date date = DateUtil.parse(value, true);
				if (date != null) {
					value = DateUtil.TSDateFormat.format(date);
