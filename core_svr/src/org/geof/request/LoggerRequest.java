...
public class LoggerRequest extends Request {
	...
	protected boolean readContents() {
		...
		try {
			...
			_jwriter.writePair(DATA, GLogger.getContents(tail, isInit));
	...
	protected boolean read() {
		...
		try {
			...
			reply.put(LEVEL, GLogger.getLogLevel());
			reply.put(FILEPATH, GLogger.getFilePath());
			reply.put(CONTENT, GLogger.getContents(tail, isInit));
	...
	private boolean setLevel() {
		...
		try {
			if (_data.has(LEVEL)) {
				GLogger.forceWrite("Logger.setlevel:" + _data.getString(LEVEL));
				GLogger.setLogLevel(_data.getInt(LEVEL));
				JSONObject data = new JSONObject();
				data.put("Log Level ", GLogger.getLogLevel());
				addData(data);
				rtn = true;
			}
		} catch (Exception e) {
	...
	private boolean runTest() {
		...
		try {
			...
			GLogger.forceWrite("Log Level: " + GLogger.getLogLevel());
			GLogger.verbose("Verbose test");
			GLogger.debug("Debug test");
			GLogger.info("Info test");
			GLogger.warn("Warn test");
			GLogger.error("Error test");
			JSONObject data = new JSONObject();
