...
//				if (sloc == null) {
//					GLogger.error("Storage location not found : " + si);
//				}				
//			}
			...
			if (_fields.has(UPLOADID)) {
				_jwriter.writePair(UPLOADID, _fields.getInt(UPLOADID));
			}
	...
	private boolean update() {
		...
		try {
			...
			StorageMgr sm = StorageMgr.instance();
			String filepart = null;
			if (_fields.has(FILEPART)) {
				filepart = _fields.getString(FILEPART);
			} else {
				filepart = _transaction.getPayload(String.valueOf(this.getRequestID()));
			}
			boolean written = sm.writeFilePart(filepart, fileindex, sendname);
			...
			_jwriter.closeObject();
			FileUpload fu = sm.getFileUpload(sendname);
			if (fu != null) {
...
//				GLogger.debug("UploadRequest.complete - " + sendname);
