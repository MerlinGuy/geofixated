...
public class AuthorityMgr {
	...
	final static String _sqlUE = "SELECT ue.*, e.name FROM ugroup_entity ue, entity e WHERE e.id = ue.entityid ORDER BY e.name";
	...
	public static boolean initialize(DBInteract dbi, boolean allowAll, boolean readOnly) {
		GLogger.writeInit("*** AuthorityManager.initialize started");
		if (dbi == null) {
			GLogger.error("PermissionManager.initialize : dbConn = null");
			return false;
		}
	...
	private static boolean loadUgroups(DBInteract dbi) {
		try {
			GLogger.writeInit(" --- AuthorityMgr.loadUgroups called");
...
//			printUgroups();
	...
	public static boolean hasPermission(GSession session, String requestname, String strAction) {
		try {
			Integer action = ACTIONS.get(strAction);
			if (action != null) {
				List<Integer> ugroups = session.getUGroups();
				if (ugroups != null) {
					for (Integer ugroupid : ugroups) {
						boolean[] perms = _ugroups.get(ugroupid).get(requestname);
				        if (perms != null && perms[action]) {
				        	return true;
				        }
					}
				}
			} else {
				GLogger.error("AuthorityMgr ERROR: unknown action attempted: " + session.getUsrID() + " - " + requestname + " : " + strAction);
			}
		} catch (Exception e) {
			GLogger.error(e);
		}
...
//			printUserPermissions(ugroups);
			...
			GLogger.error("requestname: %s, actionname: %s", entityname, actionname);
			GLogger.error(e);
		} finally {
			if (ps != null) {
	...
	public static JSONObject convertPermissions(ArrayList<Integer> ugroups) {
		try {
			...
			List<String> keys = new ArrayList<String>( mPermissions.keySet() );
	...
	public static void printUgroups() {
		...
		while (ugroupids.hasNext()) {
			...
			while (enames.hasNext()) {
				...
				GLogger.debug(ugroupid + ", " + ename + ' ' + bray[0] + ' ' + bray[1]
	...
	public static void printUserPermissions(ArrayList<Integer> ugroups) {
		...
		for (String entityname : keys) {
			...
			GLogger.debug(entityname + ' ' + bray[0] + ' ' + bray[1]
					 + ' ' + bray[2] + ' ' + bray[3] + ' ' + bray[4]);
		}
	}
