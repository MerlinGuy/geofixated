...
public class StorageMgr {
	...
	public boolean initialize(DBInteract dbi) {
		GLogger.writeInit("*** StorageMgr.initialized");
		if (dbi == null) {
			GLogger.error("StorageManager.initialize : dbi = null");
			return false;
		}
	 ...
	 * @return Returns true if no error occurred otherwise false
	 */
	public String update(DBInteract dbi, JSONObject data) {
		try {
			...
			int quota = fields.optInt(QUOTA,-1);
			if (quota < 0) {
				fields.put(QUOTA, 0);
			}
...
//					if (path == null) {
//						GLogger.error("Null alias path in apache2.conf");
//						return "Null alias path in apache2.conf ";
//					} else if (!path.equalsIgnoreCase(systemdir)) {
//						GLogger.error("Existing alias path in apache2.conf mismatch");
//						return "Existing alias path in apache2.conf mismatch";
//					}
//				} else {
//					boolean rtn = acm.AppendAlias(alias, systemdir);
//					if (!rtn) {
//						GLogger.error("Apache2 alias in apache2.conf failed for " + alias + " : " + systemdir);
...
//			}
	...
	public String delete(DBInteract dbi, JSONObject data) {
		...
		try {
			...
			if (file.exists()) {
				// GLogger.verbose("file count in systemdir " + file.list().length);
	...
	public long getIdByDirectory(String systemdir, boolean reportFailure) {
		...
		if (reportFailure) {
			GLogger.error("StorageLocation for directory: " + systemdir + " not found.");
		}
		return -1;
	}
	...
	public StorageLocation create(String name, String systemdir, boolean active, String desc) {
		...
		if (!file.exists()) {
			if (!file.mkdir()) {
				GLogger.error("StorateLocation.create failed:" + systemdir);
				return null;
			}
		}
		// TODO: finish this
	...
	public boolean deleteFileUpload(FileUpload fileupload) {
		...
		if ( ! _fileUploads.containsKey(sendname)) {
			GLogger.debug("StorageManager.deleteFileUpload missing sendname:"  + sendname);
			//TODO: create error
			return false;
		}
...
//			GLogger.debug(sendname + ", size: " + filepart.length());
			byte[] decoded = Base64.decode(filepart);
			fos.write(decoded);
//			GLogger.debug(sendname + " decode size " + decoded.length);
...
//			deleteFileUpload(fu);
	...
	public synchronized boolean moveFromStaged(String stage, String destDir, String filename) {
		...
		if (file.exists()) {
			...
			GLogger.error("moveFromStaged: " + stage + filename + " does not exist");
			return false;
		}
	}
