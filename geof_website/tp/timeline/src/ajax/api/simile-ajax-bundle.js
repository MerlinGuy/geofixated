...
}if(!handler.uuid){handler.uuid=this.uuid++;
...
elem=null;
},uuid:1,global:{},remove:function(elem,types,handler){if(elem.nodeType==3||elem.nodeType==8){return ;
...
type=parts[0];
if(events[type]){if(handler){delete events[type][handler.uuid];
...
},proxy:function(fn,proxy){proxy.uuid=fn.uuid=fn.uuid||proxy.uuid||this.uuid++;
return proxy;
...
opt.complete=function(){if(opt.queue!==false){jQuery(this).pop();
